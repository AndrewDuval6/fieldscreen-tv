...
function director() {
  ...
  q('#ez-dir-moments').innerHTML = btn('Find broadcast ↗', `data-nfl-watch="${game.id}"`) + btn('Game details', `data-nfl-detail="${game.id}"`) + btn('Full schedule', 'data-nfl-view="schedule"');
...
root.fieldscreenNfl = {
  ...
  feed(slot, headerHTML) {
    ...
    const body = game ? `<div class="fs-watch-scorecard">${scoreboard(game)}<span>${esc(game.state === 'pre' ? localTime(game.date) : game.detail)}</span><div>${pitch(game, 'watch-' + slot)}</div>${btn('Find broadcast ↗', `data-nfl-watch="${game.id}" data-watch-slot="${slot}"`)}</div>` : source === 'standings' ? `<div class="fs-watch-data"><h3>NFL STANDINGS</h3>${state.standings?.divisions.map(d => `<p>${esc(d.name)}</p>${d.entries.map(e => `<div><span>${esc(e.team.abbr)}</span><b>${esc(val(e.stats,'wins'))}–${esc(val(e.stats,'losses'))}–${esc(val(e.stats,'ties'))}</b></div>`).join('')}`).join('') || '<p>Loading standings…</p>'}</div>` : `<div class="fs-watch-data"><h3>LEAGUE SCOREBOARD</h3>${state.board?.games.map(g => `<button data-nfl-detail="${g.id}"><span>${esc(g.away.abbr)} @ ${esc(g.home.abbr)}<small>${esc(g.state === 'pre' ? localTime(g.date) : g.detail)}</small></span><b>${esc(dash(g.away.score))} : ${esc(dash(g.home.score))}</b></button>`).join('') || '<p>Connecting to ESPN…</p>'}</div>`;
...
root.addEventListener('click', event => {
  ...
  if (panel || b.dataset.nflGame || b.dataset.nflDetail || b.dataset.nflWatch || b.dataset.nflTeam || b.dataset.nflConference || b.dataset.detailTab || b.dataset.statsCategory || b.dataset.fieldPage || b.hasAttribute('data-nfl-refresh') || b.hasAttribute('data-nfl-current') || ['ez-dir-auto','ez-dir-pin','ez-dir-play','ez-dir-detail'].includes(b.id)) {
    ...
    else if (b.dataset.nflGame || b.dataset.nflDetail) { state.selected = b.dataset.nflGame || b.dataset.nflDetail; state.auto = false; state.detail = null; state.detailError = ''; navigate(b.dataset.nflDetail ? 'game' : 'gameday'); }
