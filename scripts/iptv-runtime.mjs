...
function renderGames() {
  ...
  const html = games.length ? games.map(({game, matches, ready}) => `<article class="fs-auto-game"><div><small>${game.live ? 'LIVE NOW' : escape(when(game.date))} · ${escape(game.network || game.league?.toUpperCase() || 'Sports')}</small><strong>${game.league==='mma'?escape(game.eventName):escape(game.away.fullName)+' <span>at</span> '+escape(game.home.fullName)}</strong><p>${ready ? 'Matchup confirmed in your TV guide' : matches.length ? `${matches.length} possible channel${matches.length === 1 ? '' : 's'} · ${escape(matches[0].confidence)}` : guideBusy ? 'Reading your guide…' : 'No matching coverage in the available guide'}</p></div><button class="ez-button ${ready ? 'ez-primary' : ''}" data-coverage-game="${escape((game.league || 'nfl') + ':' + game.id)}">${ready ? 'Watch game ↗' : matches.length ? 'Check coverage' : 'Find coverage'}</button></article>`).join('') : '<div class="fs-no-channels"><strong>No upcoming games loaded</strong><p>Games appear here as the schedule loads. Your channels remain available under All channels.</p></div>';
