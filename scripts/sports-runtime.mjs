...
picker.id = 'fs-sport-select'; picker.setAttribute('aria-label','Choose sport'); picker.innerHTML = '<option value="nfl">NFL</option><option value="mlb">MLB</option><option value="mma">MMA</option>';
...
function switchSport(league) {
  ...
  root.classList.toggle('fs-mma-active',league === 'mma');
...
root.fieldscreenSports = {
  ...
  teamLeagues: () => [...modules].filter(([,m])=>m.teamFavorites!==false).map(([league])=>league),
  ...
  feed(slot,html) { const source = String(api.state.slots[slot]); const league = source.startsWith('mma:') ? 'mma' : source.startsWith('mlb:') ? 'mlb' : source.startsWith('game:') || source.startsWith('nfl:') ? 'nfl' : active; return modules.get(league)?.feed(slot,html); },
