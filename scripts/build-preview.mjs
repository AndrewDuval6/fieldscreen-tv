...
for (const name of ['sports','nfl','mlb','mma','recorder','favorites','display']) await build({ entryPoints: [new URL(`./${name}-runtime.mjs`, import.meta.url).pathname], outfile: new URL(`${name}-runtime.js`, output).pathname, bundle: true, format: 'iife', target: 'chrome120' });
